...
listItem* createKtreeItem(double* value, unsigned int valueLenght) {
	...
	newItem->value = malloc(sizeof(double) * valueLenght);
	newItem->fatball = 0;
...
char searchRec(listItem* root, double* point, unsigned int depth, unsigned int dimension) {
	...
	if (arePointsSame(root->value, point, dimension)) return 1;
	...
	if (point[cd] < root->value[cd]) return searchRec(root->left, point, depth + 1, dimension);
